...
const Satting = () => {
  ...
  const handlePasswordUpdate = async () => {
    ...
    try {
      const projectId = "62b02tyexb5i";
      const apiUrl = `https://academics.newtonschool.co/api/v1/user/updateMyPassword`;
      const response = await fetch(apiUrl, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          projectId: projectId,
          Authorization: `Bearer ${jwttoken}`, // Add the 'Bearer' keyword and space before the token
        },
        body: JSON.stringify({
          name: username,
          email: email,
      ...
      if (response.ok) {
        ...
        toast.error(
          "Password update failed. Please ensure you've entered the correct current password and try again. If the issue persists, please contact our support for assistance."
        );
      }
    } catch (error) {
  ...
  return (
    <div className="profile-container">
      <h4>My Account</h4>
      <div className="detailspasswordchange">
        <Avatar
          alt="Profile Image"
          src={
            login
              ? `https://ui-avatars.com/api/?background=191D88&color=fff&name=${localStorage.getItem(
                  "username"
                )}%&size=128`
      ...
      {login ? (
        <Button
          ...
          startIcon={<ExitToApp sx={{ color: "red" }} />}
        ...
        <NavLink to="/login" className="sattingloginbtn">
          Login
        </NavLink>
      )}
    </div>
